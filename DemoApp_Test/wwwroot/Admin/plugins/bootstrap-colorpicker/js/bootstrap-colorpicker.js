...
var ColorItem = function () {
  ...
  _createClass(ColorItem, [{
     ...
     *
     * @param {int} value double from 0.0 to 1.0
     */
...
cs.get.hsl = function (string) {
	...
	if (match) {
		...
		var h = (parsedouble(match[1]) + 360) % 360;
		var s = clamp(parsedouble(match[2]), 0, 100);
		var l = clamp(parsedouble(match[3]), 0, 100);
...
cs.get.hwb = function (string) {
	...
	if (match) {
		var alpha = parsedouble(match[4]);
		var h = ((parsedouble(match[1]) % 360) + 360) % 360;
		var w = clamp(parsedouble(match[2]), 0, 100);
		var b = clamp(parsedouble(match[3]), 0, 100);
		var a = clamp(isNaN(alpha) ? 1 : alpha, 0, 1);
		return [h, w, b, a];
	}
